...
//nolint:lll
var iamPolicyDocument = pschema.ObjectTypeSpec{
	Description: `You manage access in AWS by creating policies and attaching them to IAM identities or AWS resources. A policy is an
		...
		"Statement": {
			...
			TypeSpec: pschema.TypeSpec{
				...
				Items: &pschema.TypeSpec{
					Ref: "#/types/" + string(awsType(iamMod, "documents", "PolicyStatement")),
				},
			},
		},
	},
...
var iamPolicyStatement = pschema.ObjectTypeSpec{
	Description: `The Statement element is the main element for a policy. This element is required. It can include multiple elements
...
`,
	Properties: map[string]pschema.PropertySpec{
		...
		"Principal": {
			Description: `Indicate the account, user, role, or federated user to which you would like to allow or deny access. If you are
...
`,
			TypeSpec: pschema.TypeSpec{
				OneOf: []pschema.TypeSpec{
					...
					pschema.TypeSpec{
						Ref: "#/types/" + string(awsType(iamMod, "documents", "AWSPrincipal")),
					},
					pschema.TypeSpec{
						Ref: "#/types/" + string(awsType(iamMod, "documents", "ServicePrincipal")),
					},
					pschema.TypeSpec{
						Ref: "#/types/" + string(awsType(iamMod, "documents", "FederatedPrincipal")),
					},
				},
			},
		},
...
var iamAWSPrincipal = pschema.ObjectTypeSpec{
	Description: `When you use an AWS account identifier as the principal in a policy, the permissions in the policy statement can be
...
var iamServicePrincipal = pschema.ObjectTypeSpec{
	Description: `IAM roles that can be assumed by an AWS service are called service roles. Service roles must include a trust policy.
...
var iamFederatedPrincipal = pschema.ObjectTypeSpec{
	Properties: map[string]pschema.PropertySpec{
		"Federated": {
			TypeSpec: pschema.TypeSpec{
				OneOf: []pschema.TypeSpec{
